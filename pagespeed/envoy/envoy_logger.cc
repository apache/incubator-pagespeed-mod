...
int PagespeedLogSink::getPagespeedLogLevel(spdlog::level::level_enum log_level) {
  switch (log_level) {
...
void PagespeedLogSink::log(absl::string_view msg) {
  pagespeed_message_handler_->Message(static_cast<MessageType>(getPagespeedLogLevel(log_level_)),
                                       "%s", std::string(msg).c_str());
}
