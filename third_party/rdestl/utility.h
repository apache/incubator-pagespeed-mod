...
void copy(const T* first, const T* last, T* result, int_to_type<false>) {
  ...
  while (first != last) *localResult++ = *first++;
}
template <typename T>
void copy(const T* first, const T* last, T* result, int_to_type<true>) {
  const size_t n = reinterpret_cast<const char*>(last) -
                   reinterpret_cast<const char*>(first);
  Sys::MemCpy(result, first, n);
}

template <typename T>
RDE_FORCEINLINE void move_n(const T* from, size_t n, T* result,
                            int_to_type<false>) {
  for (int i = int(n) - 1; i >= 0; --i) result[i] = from[i];
}
template <typename T>
RDE_FORCEINLINE void move_n(const T* first, size_t n, T* result,
                            int_to_type<true>) {
  Sys::MemMove(result, first, n * sizeof(T));
}

template <typename T>
RDE_FORCEINLINE void move(const T* first, const T* last, T* result,
                          int_to_type<false>) {
  result += (last - first);
...
void copy_construct_n(const T* first, size_t n, T* result, int_to_type<false>) {
  for (size_t i = 0; i < n; ++i) new (result + i) T(first[i]);
}
template <typename T>
...
void destruct_n(T* first, size_t n, int_to_type<false>) {
  ...
  // sizeof(first);
  for (size_t i = 0; i < n; ++i) (first + i)->~T();
}
template <typename T>
RDE_FORCEINLINE void destruct_n(T*, size_t, int_to_type<true>) {
...
template <typename T>
RDE_FORCEINLINE void destruct(T*, int_to_type<true>) {
  // Nothing to do, no destructor needed.
}
...
template <typename T>
RDE_FORCEINLINE void construct(T*, int_to_type<true>) {
  // Nothing to do
}

template <typename T>
RDE_FORCEINLINE void copy_construct(T* mem, const T& orig, int_to_type<false>) {
  new (mem) T(orig);
}
template <typename T>
RDE_FORCEINLINE void copy_construct(T* mem, const T& orig, int_to_type<true>) {
  mem[0] = orig;
}
...
void construct_n(T* to, size_t count, int_to_type<false>) {
  // sizeof(to);
  for (size_t i = 0; i < count; ++i) new (to + i) T();
}
template <typename T>
