...
function usage {
  echo -n "create_distro_tarball.sh [ --checkout_dir dir ] tarball"
...
# It's tarball time!
# TODO(morlovich): Fix giflib.gyp to support proper external depends.
...
tar cj --dereference --exclude='.git' --exclude='.svn' --exclude='.hg' -f $TARBALL \
    ...
    $DIR/src/third_party/gflags \
    $DIR/src/third_party/giflib \
