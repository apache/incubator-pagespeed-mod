...
class SerfFetch : public PoolElement<SerfFetch> {
 ...
 private:
  ...
  apr_status_t HandleSSLCertValidation(
    ...
    // Note that HandleSSLCertValidation can be called multiple times for a
    // single request.  As far as I can tell, there is value in recording only
    // one of these.  For now, I have set up the logic so only the last error
    // will be printed lazilly, in ReadHeaders.
    ...
    if (ssl_error_message_ == NULL && async_fetch_ != NULL) {
      if (// If cert is null that means we're being called via
          // SSLCertChainError.  We only need to check the host name matches
          // when being called via SSLCertError, in which case cert won't be
          // null.
        cert != NULL &&
          // No point in checking the host if we're allowing self-signed or a
          // made up CA, since people can forge whatever they want and often
          // don't bother to make the name match.
          !fetcher_->allow_self_signed() &&
          !fetcher_->allow_unknown_certificate_authority()) {

        DCHECK(serf_ssl_cert_depth(cert) == 0) <<
            "Serf should be filtering out intermediate certs before "
            "hitting us.";
    ...
    // HandleSSLCertValidation is called multiple times when there is an error,
